  ...
  useEffect(() => {
    ...
    const render = time => {
      ...
      cardRefs.current.forEach((card, index) => {
        ...
        const labelProgress = smoothstep(0.72, 0.98, focus) * opacity;
        card.style.setProperty('--spiral-label-progress', labelProgress.toFixed(3));
      });
        ...
        {normalizedItems.map((item, index) => {
          ...
          return (
            ...
            >
              ...
              />
              {item.label && (
                <span className="infinite-spiral__label-group">
                  <svg className="infinite-spiral__label-path" viewBox="0 0 48 16" aria-hidden="true">
                    <path d="M1 8C22 8 30 4 47 8" pathLength="1" />
                  </svg>
                  <span className="infinite-spiral__label">{item.label}</span>
                </span>
              )}
            </Card>
          );
